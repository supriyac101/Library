...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            $page->drawText(
                Mage::helper('sales')->__('ORDER # ') . $order->getRealOrderId(), 465, ($top -= 15), 'UTF-8'
            );
        }
        ...
        $image = Zend_Pdf_Image::imageWithPath($image);
        //$image = Zend_Pdf_Image::imageWithPath($image);
        $top         = 775; //top border of the page
        $widthLimit  = 270; //half of the page width
        $heightLimit = 150; //assuming the image is not a "skyscraper"
        $width       = $image->getPixelWidth();
        $height      = /*$image->getPixelHeight()*/ '40px';
		...
		die(); */
       		...
       		//$page->drawImage($image, 35, ($docHeader[1] - 15), 400, 100);
			//$page->drawImage($image, 771 , 830 , 25 , 175);
		$page->drawImage($image, $x1, $y1, $x2, $y2);
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(1));
        $page->drawRectangle(25, ($top - 25), 570, $top - 55 - $addressesHeight);
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page, 10);
        $this->y = $top - 45;
        $addressesStartY = $this->y;
		
		/* Custom code for Facility Name */
        ...
        if($fnorder->getCustomerId() === NULL){
            ...
            $_customerEmail = $customer->getEmail();
        }
        $facilityname = substr($fncustomer, 0, 25);
        $page->drawText(strtoupper($facilityname), 35, $this->y, 'UTF-8');
        /*Custom code end for Facility Name */
        //echo "<pre>";
        //print_r($address); exit;
        $customerAddressId = $customer->getDefaultBilling(); //oder getDefaultShipping
        if ($customerAddressId) {
            $address = Mage::getModel('customer/address')->load($customerAddressId);
            $companyname = $address->getCompany();
            $cusname = $address->getName();
           // $cusemail = "E-mail: ".$_customerEmail;
            $cusstreetarray = $address->getStreet();
            $cusstreetfull = ucfirst(implode(",", $cusstreetarray));
            $cuscity = ucfirst($address->getCity()).", ";
            $state = $address->getRegion();
            if($state){ $cusstate = ucfirst($address->getRegion()).", "; }
            $cuspostcode = ucfirst($address->getPostcode());
            $cusregionfull= $cuscity.$cusstate.$cuspostcode;
            $cuscountryid = Mage::getModel('directory/country')->loadByCode($address->getCountryId());
            //$cuscountry = $cuscountryid->getName();
            $custel = $address->getTelephone();
            //echo ("<pre>");
            //print_r($address);
            $addressarray = array($cusstreetfull , $cusregionfull , $cuscountry , $custel, $cusemail, $cusname);
        }
        foreach ($addressarray as $addpart) {
            if($addpart){
            $page->drawText(strip_tags(ltrim(strtoupper($addpart))), 35, $this->y -15, 'UTF-8');
            $this->y -= 15;
            }
        }
        //print_r($customerAddressId);
        //exit;
        /*
        foreach ($billingAddress as $value){
            
            if ($value !== '') {
                if($value !== $companyname && $value !== $cusname){
                    $text = array();
                    foreach (Mage::helper('core/string')->str_split($value, 45, true, true) as $_value) {
                        $text[] = $_value;
                    }
                    ...
                    foreach ($text as $part) {
                        if($part){
                        $page->drawText(strip_tags(ltrim($part)), 35, $this->y -10, 'UTF-8');
                        $this->y -= 12;
                        }
                    }
                }
        ...
        //$page->drawText("E-mail: ".$_customerEmail, 35, $this->y -15, 'UTF-8');
        //$page->drawText($cusname, 35, $this->y -15, 'UTF-8');

        $addressesEndY = $this->y -15;
		
        if (!$order->getIsVirtual()) {
            $this->y = $addressesStartY;
            $facilityname = substr($fncustomer, 0, 25);
            $page->drawText(strtoupper($facilityname), 285, $this->y, 'UTF-8');
            
            
            $shippingId = $fnorder->getShippingAddress()->getId();

            // Get shipping address data using the id
            $shipaddress = Mage::getModel('sales/order_address')->load($shippingId);
            //echo "<pre>";
            //print_r($shipaddress); exit;
            //$customerAddressId = $customer->getDefaultBilling(); //oder getDefaultShipping
            //if ($shipaddress) {
                //$address = Mage::getModel('customer/address')->load($customerAddressId);
                $companynameship = $shipaddress->getCompany();
                $cusnameship = $shipaddress->getName();
                //$cusemailship = "E-mail: ".$_customerEmail;
                $cusstreetarrayship = $shipaddress->getStreet();
                $cusstreetfullship = ucfirst(implode(",", $cusstreetarrayship));
                $cuscityship = ucfirst($shipaddress->getCity()).", ";
                $stateship = $shipaddress->getRegion();
                if($stateship){ $cusstateship = ucfirst($shipaddress->getRegion()).", "; }
                $cuspostcodeship = ucfirst($shipaddress->getPostcode());
                $cusregionfullship= $cuscityship.$cusstateship.$cuspostcodeship;
                $cuscountryidship = Mage::getModel('directory/country')->loadByCode($shipaddress->getCountryId());
                //$cuscountryship = $cuscountryid->getName();
                $custelship = $shipaddress->getTelephone();
                //echo ("<pre>");
                //print_r($address);
                $addressarrayship = array($cusstreetfullship , $cusregionfullship , $custelship, $cusnameship);
            //}
            foreach ($addressarrayship as $partship) {
                $page->drawText(strip_tags(ltrim(strtoupper($partship))), 285, $this->y -15, 'UTF-8');
                $this->y -= 15;
            }
            ...
            foreach ($shippingAddress as $value){
                if ($value!=='') {
                    ...
                    foreach ($text as $part) {
                        $page->drawText(strip_tags(ltrim($part)), 285, $this->y -10, 'UTF-8');
                        $this->y -= 12;
                    }
                    }
                }
            }
            $page->drawText($cusname, 285, $this->y -18, 'UTF-8');
            $page->drawText("E-mail: ".$_customerEmail, 285, $this->y -7, 'UTF-8');
            */
            $addressesEndY = min($addressesEndY, $this->y);
            $this->y = $addressesEndY -15;
        ...
        $collection = Mage::getModel('creditsave/creditsave')->getCollection()->addFieldToFilter('order_entiry_id', $order_id)->getData();
        //echo "<pre>"; print_r($collection); exit;
        $_cardNo = $collection[0]['card_no'];
        ...
        foreach ($payment as $value){
            if (trim($value) != '') {
                ...
                foreach (Mage::helper('core/string')->str_split($value, 45, true, true) as $_value) {
                    //echo "<pre>";
                    //print_r($_value); exit;
                    if (preg_match('/Credit Card Number:/',$_value)){
                        if($_cardNo){
                            $page->drawText(strip_tags(trim("CREDIT CARD NUMBER: ".$_cardNo)), $paymentLeft, $yPayments, 'UTF-8');
                        }else{
                            $page->drawText(strip_tags(trim(strtoupper($_value))), $paymentLeft, $yPayments, 'UTF-8');
                        }
                    }else{
                        $page->drawText(strip_tags(trim(strtoupper($_value))), $paymentLeft, $yPayments, 'UTF-8');
                    }
		...
		//exit;
        /* Add CVV no to payment info */
        //$order_id = $order->getId();
        //$collection = Mage::getModel('creditsave/creditsave')->getCollection()->addFieldToFilter('order_entiry_id', $order_id)->getData();
        //echo "<pre>"; print_r($collection); exit;
        $_paymentMethod = $order->getPayment()->getData('method');
        //echo "<pre>"; print_r($_paymentMethod); exit;
        if($_paymentMethod == "ccsave"){
            $cvvNo = 'CVV NO. '.$collection[0]['card_cvv'];
            $page->drawText($cvvNo, 35, $yPayments, 'UTF-8');
            $expDate = 'EXPIRATION DATE: '.$collection[0]['card_exp_month']."-".$collection[0]['card_exp_year'];
            $page->drawText($expDate, 35, $yPayments -= 15, 'UTF-8');
        }
        /* Add CVV no to payment info */
        
        /* Add PO # to payment info */
        //if($_paymentMethod == "cashondelivery"){
            $povalueText = 'PO# '.$order->getOrderPo();
            //$povalueText = preg_replace('/<br[^>]*>/i', "\n", $povalueText);
            if($_paymentMethod == 'cashondelivery'){
                $page->drawText($povalueText, 35, $yPayments -= 3, 'UTF-8');
            } else {
                $page->drawText($povalueText, 35, $yPayments -= 15, 'UTF-8');
            }
            
            //$page->drawText("", 35, $yPayments -= 10, 'UTF-8');
        //}
		/* Add PO # to payment info */
        
        /* Add coupon code for free shipping*/
        $couponCode = $order->getData('coupon_code');
        $couponCodeName = $order->getData('coupon_rule_name');
        //echo $couponCodeName; exit;
        if($couponCode){
            $page->drawText("COUPON CODE APPLIED FOR ".strtoupper($couponCodeName).": ", 35, $yPayments -= 15, 'UTF-8');
            $this->_setFontRegular($page, 30);
            $page->setFillColor(new Zend_Pdf_Color_Html('#FF0000'))->drawText($couponCode, 35, $yPayments -= 25, 'UTF-8');
            $page->drawText("", 35, $yPayments -= 15, 'UTF-8');
        }
        /* Add coupon code for free shipping*/
    ...
    public function insertDocumentNumber(Zend_Pdf_Page $page, $text)
    {
        ...
        $this->_setFontRegular($page, 10);
        $docHeader = $this->getDocHeaderCoordinates();
        $page->drawText($text, 35, $docHeader[1] - 30, 'UTF-8');
    }
    ...
    protected function insertTotals($page, $source){
        ...
        foreach ($totals as $total) {
            $total->setOrder($order)
                ->setSource($source);
            if ($total->canDisplay() || $total->getTitle() == "Shipping & Handling") {
                $total->setFontSize(10);
                foreach ($total->getTotalsForDisplay() as $totalData) {
                    if($total->getTitle() == "Shipping & Handling"){
                        if($discountAmount == 0 && $couponCode){
                            $lineBlock['lines'][] = array(
                                ...
                                array(
                                    'text'      => strtoupper($totalData['amount']),
                                    'feed'      => 565,
                                    'align'     => 'right',
                                    'font_size' => $totalData['font_size'],
                                    'font'      => 'bold'
    ...
    protected function insertCouponcode($page, $source){
        $order = $source->getOrder();
        $totals = $this->_getTotalsList($source);
        $lineBlock = array(
            'lines'  => array(),
        ...
        foreach ($totals as $total) {
            ...
            if ($total->canDisplay() || $total->getTitle() == "Shipping & Handling") {
                ...
                foreach ($total->getTotalsForDisplay() as $totalData) {
                    if($total->getTitle() == "Shipping & Handling"){
                        if($discountAmount == 0 && $couponCode){
                            $color2 = $page->setFillColor(new Zend_Pdf_Color_Html('#FF0000'));
                            $lineBlock['lines'][] = array(
                                /*array(
