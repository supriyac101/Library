...
class Mirasvit_Fpc_Block_Adminhtml_OptimalConfiguration extends Mage_Adminhtml_Block_Template
{
    ...
    public function getMaxCacheSize() {
        ...
        $maxCacheSize = (($this->getMaxNumberCacheFiles() / 10000) * 2) * 1024;
        if ($maxCacheSize < 10240) {
            $maxCacheSize = 10240;
        }
        if ($this->isRedisInstalled()) {
            $maxCacheSize = 256;
        ...
        if (($freeHddSpace === 0 || $freeHddSpace > 0)
            && $maxCacheSize > $freeHddSpace) {
            $maxCacheSize = $freeHddSpace - 500;
            $maxCacheSize = ceil($maxCacheSize);
        }
    ...
    public function getMaxNumberCacheFiles() {
        $groupCollectionSize = Mage::getModel('customer/group')->getCollection()->getSize();
        $maxNumberCacheFiles = ($this->getCategoryCount() * 20) + ($this->getProductsCount() * 2) * $groupCollectionSize;
        $maxNumberCacheFiles =  ceil($maxNumberCacheFiles / 1000) * 1000;
