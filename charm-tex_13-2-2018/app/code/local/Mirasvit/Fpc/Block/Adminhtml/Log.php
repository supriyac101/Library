...
class Mirasvit_Fpc_Block_Adminhtml_Log extends Mage_Adminhtml_Block_Template
{
    ...
    public function getCacheInfo()
    {
        ...
        if ($backend instanceof Zend_Cache_Backend_File) {
            $backend = Mirasvit_Fpc_Model_Cache::getCacheInstance()->getFrontend()->getBackend();
            $cacheDir = false;
            if (in_array('getOption', get_class_methods($backend))) {
                $cacheDir = $backend->getOption('cache_dir');
            } else {
                $backendData = (array) $backend;
                $prefix = chr(0).'*'.chr(0);
                if(isset($backendData[$prefix.'_options']['cache_dir'])) {
                    $cacheDir = $backendData[$prefix.'_options']['cache_dir'];
                }
            }            
            ...
            if ($cacheDir) {
                ...
                $cacheDirPrepared = explode('/', $cacheDir);
                $cacheDirPrepared = array_slice($cacheDirPrepared, -2);
                $cacheDir = '/' . implode('/', $cacheDirPrepared) . '/';
                $html[] = __('Cache folder: <b>%s</b>', $cacheDir);
            }
