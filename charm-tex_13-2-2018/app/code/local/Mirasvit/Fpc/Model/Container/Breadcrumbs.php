...
class Mirasvit_Fpc_Model_Container_Breadcrumbs extends Mirasvit_Fpc_Model_Container_Abstract
{
    ...
    protected $_forceCategoryId = false; //true - force generate category id for product
    ...
    protected function _isCmsPage()
    {
        ...
        $cmsPagesActions = array('cms/index_index', 'cms/page_view');
        if (in_array($action, $cmsPagesActions)) {
            return true;
        }
    ...
    {
        $isCmsPage = $this->_isCmsPage();
        $categoryId = $this->_getCategoryId($isCmsPage);
        $productId = $this->_getProductId();
    ...
    protected function _getCategoryId($isCmsPage = false)
    {
        if ($isCmsPage) {
            if (isset($_COOKIE['mfpcbreadcrumb'])) {
                Mage::getModel('core/cookie')->delete('mfpcbreadcrumb');
        ...
        if ($this->_getProductId() && !$categoryId && isset($_COOKIE['mfpcbreadcrumb'])) {
            //cookie
        ...
        //force generate category id for product
        if (!$categoryId && $this->_getProductId() && $this->_forceCategoryId) {
