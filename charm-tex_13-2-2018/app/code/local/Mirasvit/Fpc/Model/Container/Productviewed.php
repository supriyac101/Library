...
class Mirasvit_Fpc_Model_Container_Productviewed extends Mirasvit_Fpc_Model_Container_Abstract
{
    ...
    protected function _getProductIds()
    {
        ...
        if ((Mage::registry('current_product_id')
            && in_array(Mage::registry('current_product_id'), $result)
            && ($productId = Mage::registry('current_product_id')))
            || (Mage::registry('current_product')
                && in_array(Mage::registry('current_product')->getId(), $result)
    ...
    {
        preg_match('/<block type="reports\\/product_viewed"(.*?)template="catalog\\/product\\/view\\/product_viewed.phtml"\\/\\>/',$this->_definition['layout'], $productViewed);
        if (isset($productViewed[0]) && $productViewed[0]) {
            $this->_definition['layout'] =  '<layout>' . $productViewed[0] . '</layout>';
        }
