...
//		Mage::setIsDeveloperMode(true);
	...
	public function recentaddtocartAction()
	{
		ini_set('display_errors', 1);
        Varien_Profiler::enable();
		Mage::setIsDeveloperMode(true);
		...
		$qty = Mage::app()->getRequest()->getParam('qty');
		$optid = Mage::app()->getRequest()->getParam('optid');
		$optvalid = Mage::app()->getRequest()->getParam('optvalid');
		//echo "<pre>";
		//print_r($params);
		// Access our object's data and array values.
		//echo "Data is: " . $params->product . "<br>";
		$pModel = Mage::getSingleton('catalog/product')->load($product);
		$cart = Mage::getModel('checkout/cart');
		$cart->init();
		$params = array(
					'qty' => $qty,
    ...
    {
		$_dataValue = $this->getRequest()->getPost("option_val");
		$dataSku = $this->getRequest()->getPost("productSku");
		$dataId = $this->getRequest()->getPost("option_Id");
		$_productCollection = Mage::getModel('catalog/product')
							->getCollection()
							->addAttributeToSelect('*')
							//->addAttributeToFilter('status', array('gt' => 0))
							->addAttributeToFilter(array(
								array('attribute'=>'sku','like'=>"$dataSku"),
								))
							->load();
		//echo count($_productCollection);
		$proListHtml= '';
		
		//$proListHtml.= $datatext.' Model ';
		foreach($_productCollection as $product){
			//echo $product->getName();
			$productId = $product->getId();
			$loadedproduct = Mage::getModel('catalog/product')->load($productId);
			foreach($loadedproduct->getOptions() as $option) {
				if($option->getOptionId() == $dataId){
					foreach($option->getValues() as $value) {
						if($value->getOptionTypeId() === $_dataValue){
							if($value->getSku()){
								$proListHtml.= $value->getSku();	
							}else{
								$proListHtml.= '';
							}
