...
class Velocity_Supercheckout_Model_Customer extends Mage_Core_Model_Abstract
{
    /**#@+
    ...
    public function validate()
    {
        ...
        if (!Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            $errors['invalid_email'] = Mage::helper('customer')->__('Invalid email address.', $this->getEmail());
        }
        ...
        if ($password != $confirmation) {
            $errors['match_password'] = Mage::helper('customer')->__('Please make sure your passwords match.');
        }
        //Start Code Modified By Raghubendra Singh on 05-Jan-2015 to add validations for the new fields added for new or guest user
        $entityType = Mage::getSingleton('eav/config')->getEntityType('customer');
        $attribute = Mage::getModel('customer/attribute')->loadByCode($entityType, 'dob');      
        if ($attribute->getIsRequired() && '' == trim($this->getDob())) {
            $errors['dob_required'] = Mage::helper('customer')->__('Please Enter a Valid Date of Birth.');
        }
        $attribute = Mage::getModel('customer/attribute')->loadByCode($entityType, 'taxvat');
        if ($attribute->getIsRequired() && '' == trim($this->getTaxvat())) {
            $errors['tax_required'] = Mage::helper('customer')->__('The TAX/VAT number is required.');
        }
        $attribute = Mage::getModel('customer/attribute')->loadByCode($entityType, 'gender');
        if ($attribute->getIsRequired() && '' == trim($this->getGender())) {
            $errors['gender_required'] = Mage::helper('customer')->__('Gender is required.');
        }
        //End Code Modified By Raghubendra Singh on 05-Jan-2015 to add validations for the new fields added for new or guest user
