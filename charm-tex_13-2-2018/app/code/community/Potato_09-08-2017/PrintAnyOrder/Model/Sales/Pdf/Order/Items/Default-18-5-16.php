    ...
    extends Mage_Sales_Model_Order_Pdf_Items_Abstract
{
    ...
    public function draw()
    {
        ...
        if(substr($this->getSku($item), $subsku)){
            $lines[0][] = array(
                'text' => Mage::helper('core/string')->str_split(
                    substr($this->getSku($item), $subsku), 18
                ),
                'feed' => 35,
            );
        }else{
        ...
        if ($options) {
            ...
            foreach ($options as $option) {
                // draw options label
                $lines[$c][] = array(
                    'text' => Mage::helper('core/string')->str_split(strtoupper($option['label']).": ".strtoupper(htmlspecialchars_decode($option['value'])), 15, true, true),
                    'feed' => 310,
                    'align'=>'left',
                );
            $c++;
