    ...
    public function productAction()
    {
		...
		foreach ($_productCollectionOpt->getOptions() as $_option):
			...
			if($_option->getCssClass() == "step4"):
				...
				foreach ($_values as $k => $_value) {
					if($_option->getSortOrder() != 8):
					$step4 .='<div class="box1"><input type="'.$_option->getType().'" name="options['.$_option->getId().']" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" price="'.$_value->getPrice().'" /><img src="'.$_value->getImage().'" alt="'.$_value->getTitle().'" /></label></div>';
