		...
		foreach ($_productCollectionOpt->getOptions() as $_option): $i++;
			...
			if($_option->getCssClass() == "step6"):
				...
				if($_option->getType()=="radio"):
					...
					foreach ($_values as $k => $_value) {
						$step6b .='<span><input type="'.$_option->getType().'" class="radiooptnstp6 '.$validate.'" name="options['.$_option->getId().']" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" title="'.$_value->getTitle().'" /><label for="'.$_value->getTitle().'">'.$_value->getTitle().'</label></span>';
					}
					$step6b .='</div>';
				else:
					$step6 .= '<li>
							...
							<div class="input-box">
								<input type="'.$_option->getType().'" class="input-text required-entry '.$validate.' input-textStp6" value="" name="options['.$_option->getId().']" price="'.$_option->getPrice().'" />
	...
	public function reviewAction(){
		...
		$message .='
			<table class="separate">
				...
				<td><h6 class="revtbhdg">Product Price</h6></td><td>'.$option->getPrice().'</td></tr>';
