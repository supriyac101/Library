...
class Webskitters_Customization_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function productAction()
    {
		...
		foreach ($_productCollectionOpt->getOptions() as $_option):
			if($_option->getCssClass() == "step2"):
				$step2 .='<input type="'.$_option->getType().'" class="hidden" id="uploadFile" name="filename" price="'.$_option->getPrice().'" optname="options_'.$_option->getId().'_file_action" />';
			endif;
			if($_option->getCssClass() == "step3"):
				...
				foreach ($_values as $k => $_value) {
					...
					$step3 .='<li><input type="'.$_option->getType().'" class="radiooptnstp3" name="options['.$_option->getId().'][]" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" image="'.$_value->getImage().'" price="'.$_value->getPrice().'" /><label for="radio'.$_value->getId().'">'.$_value->getTitle().'</label>
					</li>';
					else
					$step3 .='<li><input type="'.$_option->getType().'" class="radiooptnstp3" name="options['.$_option->getId().']" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" image="'.$_value->getImage().'" price="'.$_value->getPrice().'" /><label for="radio'.$_value->getId().'">'.$_value->getTitle().'</label>
					</li>';
			...
			if($_option->getCssClass() == "step4"):
				...
				foreach ($_values as $k => $_value) {
					if($_option->getSortOrder() != 8):
					$step4 .='<div class="box1"><input type="'.$_option->getType().'" class="radiooptnstp3" name="options['.$_option->getId().']" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" price="'.$_value->getPrice().'" /><img src="'.$_value->getImage().'" alt="'.$_value->getTitle().'" /></label></div>';
					elseif($_option->getSortOrder() == 8):
					$step4 .='<p><input type="'.$_option->getType().'" class="radiooptnstp3" name="options['.$_option->getId().']" id="options_'.$_option->getId().'_'.$_value->getId().'" value="'.$_value->getId().'" price="'.$_value->getPrice().'" image="'.$_value->getImage().'" /><label for="options_'.$_option->getId().'_'.$_value->getId().'">'.$_value->getTitle().'</label></p>';
					endif;
				}
				if($_option->getSortOrder() != 8):
							...
							<div class="logo-box">
								...
								<div class="stpfvdn">
									...
									$step5 .='<div class="stpfvuploadbtn">
										...
										<input name="filename'.$_option->getSubtitle().'" id="filename'.$_option->getSubtitle().'" value="" class="stepfvfile" type="'.$_option->getType().'" price="'.$_option->getPrice().'">
			...
			if($_option->getCssClass() == "step6"):
				$step6 .= '<li>
							...
							<div class="input-box">
								<input type="'.$_option->getType().'" class="input-text required-entry" value="" name="options['.$_option->getId().']" />
							</div>
						</li>';
			endif;
		endforeach;
